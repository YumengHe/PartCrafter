...
def main():
    ...
    for batch in yield_forever(train_loader):
        ...
        with accelerator.accumulate(transformer):
            ...
            # ====== Per-part images -> DINOv2 embeds ======
            num_parts = batch["num_parts"]                 # [M]
            num_objects = int(num_parts.shape[0])          # M

            # 期望 batch["images"] 已经是每个 part 对应的图像，形状 [N, H, W, 3] 或 List[PIL]
            per_part_images = batch["images"]
            total_parts = int(num_parts.sum().item())

            # Add detailed debug logging for part-specific images (only for first few steps or every 100 steps)
            debug_this_step = global_update_step <= 5 or global_update_step % 100 == 0

            if debug_this_step:
                logger.info(f"[Step {global_update_step:06d}] Processing batch with {num_objects} objects, {total_parts} total parts")
                logger.info(f"[Step {global_update_step:06d}] Num parts per object: {num_parts.tolist()}")
                logger.info(f"[Step {global_update_step:06d}] Per-part images shape: {per_part_images.shape}")

            # Verify that we have exactly one image per part
            assert per_part_images.shape[0] == total_parts, f"Expected {total_parts} images, got {per_part_images.shape[0]}"

            # HF 的 BitImageProcessor 同时支持 PIL 列表或 ndarray；无需手工做 HWC->CHW
            with torch.no_grad():
                pixel_values = feature_extractor_dinov2(images=per_part_images, return_tensors="pt").pixel_values
            pixel_values = pixel_values.to(device=accelerator.device, dtype=weight_dtype)

            if debug_this_step:
                logger.info(f"[Step {global_update_step:06d}] Processed pixel values shape: {pixel_values.shape}")

            with torch.no_grad():
                image_embeds = image_encoder_dinov2(pixel_values).last_hidden_state   # [N, L, C]
            ...
            if debug_this_step:
                logger.info(f"[Step {global_update_step:06d}] Part surfaces shape: {part_surfaces.shape}")

            # Verify that images and part_surfaces have consistent first dimension (number of parts)
            assert part_surfaces.shape[0] == total_parts, f"Part surfaces and images shape mismatch: {part_surfaces.shape[0]} vs {total_parts}"

            # ====== 构造 obj_ids: 每个 part 属于哪个 object，用于CFG按物体dropout ======
            obj_ids = torch.repeat_interleave(
                torch.arange(num_objects, device=accelerator.device),
                num_parts
            )  # [N]
            ...
            # === Longest-edge per-part weighting（物体内部归一化 + 夹紧）===
            # 思路：对每个 part 取 AABB 的最长边 L_i；在“物体内部”用均值做相对归一化：
            # w_i = ((mean(L)_obj / (L_i + eps)) ** alpha)；再把 w 限幅，最后乘到每-part 标量损失上。

            # device = loss.device
            # dtype  = loss.dtype
            # eps    = torch.tensor(1e-6, device=device, dtype=dtype)

            # # 1) 为每个 part 计算 AABB 的最长边 L_i
            # coords  = part_surfaces[..., :3].to(dtype)                 # [N, P, 3]
            # extents = (coords.max(dim=1).values - coords.min(dim=1).values)  # [N, 3]
            # L       = extents.max(dim=1).values + eps                  # [N] 最长边

            # # 2) 构造每个 part 对应的物体 id（用已有的 num_objects, num_parts）
            # obj_ids = torch.repeat_interleave(
            #     torch.arange(num_objects, device=device),
            #     num_parts
            # )                                                           # [N]

            # # 3) 在物体内部做相对归一化：用“该物体的 L 均值”除以当前 part 的 L
            # L_sum   = torch.zeros(num_objects, device=device, dtype=dtype).scatter_add_(0, obj_ids, L)
            # counts  = num_parts.to(dtype)                               # [M]
            # L_mean  = L_sum / (counts + eps)                            # [M]

            # alpha   = 0.7                                               # 调参范围建议 0.5 ~ 1.0
            # w_size  = (L_mean[obj_ids] / L).pow(alpha)                  # [N] 小件(短边)→权重大

            # # 4) 夹紧，避免过大/过小的梯度放大（很重要）
            # w_min, w_max = 0.5, 2.0
            # w_size = w_size.clamp(w_min, w_max)

            # # 5) 乘到“每个 part 的标量 loss”上，然后再做全局均值反传
            # loss = loss * w_size                                       # [N]
